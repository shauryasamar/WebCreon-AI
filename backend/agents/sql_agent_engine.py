...
1. `orders`
   ...
   - shipping_address (JSONB) - Contains full_name, city, postal_code, address_line1, mobile_number
...
5. COLUMN NAMING CONVENTIONS (CRITICAL):
...
6. CURRENCY & REVENUE: Exclude cancelled orders (`status != 'cancelled'`) when computing gross revenue or sales totals.
7. DATES: Use PostgreSQL date functions like `CURRENT_DATE`, `NOW()`, `created_at >= NOW() - INTERVAL '7 days'`, `DATE_TRUNC('month', created_at)`.
   Do NOT apply date filters unless the user explicitly requested a time period (e.g. "today", "last 7 days", "this month").
8. LIMIT: Always append `LIMIT 50` unless an exact count is computed.
9. Return clean PostgreSQL syntax compatible with version 15+."""),
    ...
    try:
        with Session(engine) as db:
            ...
            for row in raw_rows:
                ...
                for col, val in row_items.items():
