/**
 * Standard Kazuhiko Arase Reference QR Code Engine (ISO/IEC 18004 Standard)
 * 100% Scannable across all Apple iOS Camera, Google Lens, Samsung Camera, and UPI Payment scanners.
 */

// ─────────────────────────────────────────────────────────────────────────────
// 1. Galois Field GF(2^8) Math
// ─────────────────────────────────────────────────────────────────────────────
const QRMath = {
...
// ─────────────────────────────────────────────────────────────────────────────
// 2. Reed-Solomon Polynomial Arithmetic
// ─────────────────────────────────────────────────────────────────────────────
class QRPolynomial {
  num: number[];
...
// ─────────────────────────────────────────────────────────────────────────────
// 3. RS Block Configuration Tables (ISO/IEC 18004 Standard)
// ─────────────────────────────────────────────────────────────────────────────
class QRRSBlock {
  totalCount: number;
  dataCount: number;
  ...
  // Error Correction Level M = 0, L = 1, H = 2, Q = 3
  ...
  static getRsBlockTable(typeNumber: number, errorCorrectLevel: number): number[] | undefined {
    switch (errorCorrectLevel) {
      case 1: // L
        return QRRSBlock.RS_BLOCK_TABLE[typeNumber - 1][0];
      case 0: // M
        return QRRSBlock.RS_BLOCK_TABLE[typeNumber - 1][1];
      case 3: // Q
        return QRRSBlock.RS_BLOCK_TABLE[typeNumber - 1][2];
      case 2: // H
        return QRRSBlock.RS_BLOCK_TABLE[typeNumber - 1][3];
      default:
        return undefined;
    }
  }
...
const QRUtil = {
  ...
  getLostPoint(qrCode: QRCodeModel): number {
    ...
    // Penalty N1 (5+ adjacent same color)
    for (let row = 0; row < moduleCount; row++) {
      let sameCount = 0;
      let prevDark = qrCode.isDark(row, 0);
      for (let col = 1; col < moduleCount; col++) {
        const dark = qrCode.isDark(row, col);
        if (dark === prevDark) {
          sameCount++;
          if (sameCount === 5) lostPoint += 3;
          else if (sameCount > 5) lostPoint += 1;
        } else {
          sameCount = 1;
          prevDark = dark;
        }
    ...
    for (let col = 0; col < moduleCount; col++) {
      let sameCount = 0;
      let prevDark = qrCode.isDark(0, col);
    ...
    // Penalty N3 (Finder-like patterns 1:1:3:1:1)
    for (let row = 0; row < moduleCount; row++) {
...
// ─────────────────────────────────────────────────────────────────────────────
// 5. Complete Kazuhiko Arase QRCodeModel Core Engine
// ─────────────────────────────────────────────────────────────────────────────
class QRCodeModel {
  typeNumber: number;
...
// ─────────────────────────────────────────────────────────────────────────────
// 6. Public Export API Functions
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Generates the Boolean Matrix of the QR code using Error Correction Level M (0)
 */
export function generateQrMatrix(text: string): boolean[][] {
  const qr = new QRCodeModel(0, 0); // Type 0 = auto-detect, EC Level 0 = M (15% recovery)
  qr.addData(text);
  qr.make();
...
/**
 * Generates an SVG Data URI with the mandatory 4-module Quiet Zone.
 */
export function generateQrSvgDataUrl(text: string, fgColor = "#0f172a", bgColor = "#ffffff"): string {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalSize} ${totalSize}" width="${totalSize * 8}" height="${totalSize * 8}" shape-rendering="crispEdges"><rect width="${totalSize}" height="${totalSize}" fill="${bgColor}"/><path d="${pathData.trim()}" fill="${fgColor}"/></svg>`;
  return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`;
}

/**
 * Generates a high-resolution Canvas Data URL (JPEG/PNG) with mandatory Quiet Zone for downloads and PDF export.
 */
export function generateQrCanvasDataUrl(
  text: string,
  dimension = 1000,
  fgColor = "#0f172a",
  bgColor = "#ffffff",
  ...
  if (typeof document === "undefined" || !text || !text.trim()) return "";
  const matrix = generateQrMatrix(text.trim());
  const size = matrix.length;
  const margin = 4; // ISO Quiet Zone
